...
def get_gradcam_overlay(base_model, face_rgb_uint8: np.ndarray) -> str:
    ...
    """
    try:
        # Preprocess input image (224, 224, 3)
        ...
        with tf.GradientTape() as tape:
            conv_outputs, predictions = grad_model(img_prep, training=False)
        ...
        # Resize heatmap to frame dimensions
        heatmap = cv2.resize(heatmap, (224, 224))
        heatmap = np.uint8(255 * heatmap)
        color_heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)

        # Overlay on original image (convert RGB to BGR for OpenCV blend)
